...
def handle_first_run(root, preferences_file="preferences.txt"):
...
def check_first_run_and_show_tutorial(root, preferences_file="preferences.txt"):
    ...
    if is_first_run(preferences_file):
        handle_first_run(root, preferences_file)
    else:
    ...
    try:
        ...
        if installed_version_tuple < current_version_tuple:
            ...
            choice = messagebox.askyesno(
                ...
                f"Ваша версия {installed_version} устарела.\n"
                f"Рекомендуется установить последнюю версию с официального сайта FreeSoft:\n"
                f"https://freesoft.ru/windows/ras-kalkuliator-7976/download\n\n"
            ...
            if choice:
                ...
                webbrowser.open_new_tab("https://freesoft.ru/windows/ras-kalkuliator-7976/download")
        else:
...
def apply_theme(root, bg_color):
    ...
    style.configure('TButton', background=analogous_colors(bg_color), foreground=contrast_color(analogous_colors(bg_color)))
...
def plot_linear_equation(a, b, c):
    ...
    ax.set_ylim(center_y - 5, center_y + 5)  # Центрирование по оси Y
...
def solve_system_of_equations(event=None):
    try:
        ...
        for equation in equations_list:
            ...
            equation = re.sub(r'(\d+)([a-zA-Z])', r'\1*\2', equation)  # Вот тут была ошибка!
        ...
        if len(expressions) < len(used_variables):
            ...
            if response:
                ...
                first_var, second_var = next(iter(eq.free_symbols)), next(iter(eq.free_symbols))
        ...
        handle_error(f"Ошибка: {e}\n", input_data=equations_str, function_name='solve_system_of_equations', lb=label_system_of_equations)
...
def calculate_statistics(stat_type):
    ...
    try:
        ...
        if stat_type == "mean":
            ...
            minimum = min(numbers)
            final_result = dynamic_precision(minimum)
        ...
        handle_error(f"Ошибка: {e}", input_data=entry_numbers.get(), function_name='calculate_statistics', lb=label_stat_result)
...
def arithmetic_operation_fractions(first_fraction, second_fraction, operation):
    ...
    try:
        ...
        handle_error("Ошибка: деление на ноль.", input_data=(first_fraction, second_fraction), function_name='arithmetic_operation_fractions')
    except ValueError as ve:
        handle_error(f"Ошибка: {ve}", input_data=(first_fraction, second_fraction), function_name='arithmetic_operation_fractions')
    except Exception as e:
        handle_error(f"Ошибка: {e}", input_data=(first_fraction, second_fraction), function_name='arithmetic_operation_fractions')
...
def process_trigonometric_function(function_type):
    ...
    try:
        ...
        trig_output.config(text=f"Результат: {result:.5f}")
        clear_errors()
        add_to_history(f"{function_type}({angle})", result)
        update_history()
    except ValueError as ve:
...
def handle_error(error_message, input_data=None, function_name=None, lb=None):
    ...
    elif "'Symbol' object has no attribute 'system'" in error_message:
        full_error_message = 'Не выполняйте код'
    error_label.insert(tk.END, full_error_message)
...
error_label = tk.Text(root, height=5, state="disabled")
